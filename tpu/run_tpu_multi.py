...
def train(device, model, tokenizer, tr_paths, d_paths):
    ...
    for epoch in range(FLAGS['num_epochs']):
        ...
        train_fn(para_train_loader.per_device_loader(device))

        curr_rouge_score = evaluate(device, model, tokenizer, d_paths)
...
def evaluate(device, model, tokenizer, dev_paths):
    set_seed(FLAGS['seed'])
    ...
    def eval_loop_fn(loader):
        for step, batch in enumerate(loader):

            input_ids, stories, summaries, attention_mask, perm_mask, target_mapping, article_names = batch
            predicted_sequence = torch.zeros((input_ids.shape[0], FLAGS['sum_len']), dtype=torch.int32)
            ...
            for predict_pos in range(FLAGS['sum_len']):
                ...
                with torch.no_grad():
                        ...
                        for i in range(FLAGS['batch_size']):
                            for previous_tokens in set(input_ids[i].tolist()):
                                ...
                                if next_token_logits[i, -1, previous_tokens] < 0:
                                    ...
                                    next_token_logits[i, -1, previous_tokens] /= FLAGS['repetition_penalty']
                    print(next_token_logits.view(input_ids.shape[0], -1))
                    ...
                    for i in range(predicted_indices.shape[0]):
                        predicted_sequence[i, predict_pos] = int(predicted_indices[i])
    ...
    eval_loop_fn(para_eval_loader.per_device_loader(device))
...
def main(rank, flags):
    ...
    if FLAGS['do_train']:
        logger.info("***** Running training *****")
        tr_losses, dev_scores, best_dev_score = train(device, model, tokenizer, train_paths, dev_paths)
    ...
    if FLAGS['do_evaluate']:
        ...
        if FLAGS['lead']:
            ...
            evaluate(device, model, tokenizer, dev_paths)
        # test set check
        else:
            logger.info("***** Running testing *****")
            evaluate(device, model, tokenizer, test_paths)
...
if __name__ == "__main__":
    ...
    xmp.spawn(fn=main, args=(vars(args),), nprocs=8)
