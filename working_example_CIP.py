...
def _load_and_prepare_input(path, article_name, tokenizer, sum_len):
    ...
    with open("articles/{}_article.txt".format(article_name), "w") as article_file:
...
def main(path, sum_len, version, tokenizer, model):
    ...
    with open("logs/{}_log_{}.txt".format(article_name, version), "w") as log:
        ...
        for predict_pos in range(sum_len):
            ...
            with open("summaries/{}_summary_{}.txt".format(article_name, version), "w") as summary_file:
        ...
        duration = (toc-tic)/60
        log.write("Article {} took {} minutes".format(article_name, duration))
